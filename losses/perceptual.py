...
class PerceptualLoss(nn.Module):
    """
    Perceptual losses using features from pretrained deep neural networks trained. The function supports networks
    ...
    Args:
        ...
        is_fake_3d: if True use 2.5D approach for a 3D perceptual losses.
    ...
    def __init__(
        ...
        if spatial_dims not in [2, 3]:
            raise NotImplementedError("Perceptual losses is implemented only in 2D and 3D.")
    ...
    def _calculate_axis_loss(self, input: torch.Tensor, target: torch.Tensor, spatial_axis: int) -> torch.Tensor:
        """
        Calculate perceptual losses in one of the axis used in the 2.5D approach. After the slices of one spatial axis
        is transformed into different instances in the batch, we compute the losses using the 2D approach.
...
class MedicalNetPerceptualSimilarity(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        Compute perceptual losses using MedicalNet 3D networks. The input and target tensors are inputted in the
        ...
        value from the spatial dimensions to obtain the perceptual losses.
